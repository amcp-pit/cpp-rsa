...
TEST(Create, constructor_hex_string) {
  ...
  EXPECT_EQ(BigInteger("1234567890ABCDEF", biginteger_base::hex),
            BigInteger("1311768467294899695"));
  EXPECT_EQ(BigInteger("FEDCBA987654321", biginteger_base::hex),
            BigInteger("1147797409030816545"));
}
...
TEST(Converting, string_conv) {
  ...
  EXPECT_EQ("670442572800", to_string(BigInteger("670442572800")));
  EXPECT_EQ("354224848179261915075",
            to_string(BigInteger("354224848179261915075")));
  EXPECT_EQ("265252859812191058636308480000000",
            to_string(BigInteger("D13F6370F96865DF5DD54000000",
                                 biginteger_base::hex)));
  EXPECT_EQ(
      "280571172992510140037611932413038677189525",
      to_string(BigInteger("280571172992510140037611932413038677189525")));
}
...
TEST(Converting, hex_conv) {
  ...
  EXPECT_EQ("1234567890ABCDEF",
            to_hex(BigInteger("1234567890ABCDEF", biginteger_base::hex)));
  EXPECT_EQ("FEDCBA987654321",
            to_hex(BigInteger("FEDCBA987654321", biginteger_base::hex)));
  EXPECT_EQ("9C197DCC00", to_hex(BigInteger("9C197DCC00", biginteger_base::hex)));
  EXPECT_EQ("D13F6370F96865DF5DD54000000",
            to_hex(BigInteger("265252859812191058636308480000000")));
}
...
TEST(Arithmetic, one_plus_zero) {
  ...
  EXPECT_EQ(BigInteger(1),
            BigInteger(1) + 0);  // implicit conversion from int must work
  EXPECT_EQ(BigInteger(1), 0 + BigInteger(1));
}

TEST(Arithmetic, one_plus_one) {
  EXPECT_EQ(BigInteger(2), BigInteger(1) + BigInteger(1));
  EXPECT_EQ(BigInteger(2),
            BigInteger(1) + 1);  // implicit conversion from int must work
...
TEST(Arithmetic, add_long) {
  ...
  BigInteger c(
      "100000000000000000000000000000000000000000000000000001000000000000000000"
      "00000000000000000000");
...
TEST(Arithmetic, add_long_signed2) {
  ...
  BigInteger b("100000000000000000000000000000000000000");
  BigInteger c(
      "-99999999999999999999999999999999999999999999999999990000000000000000000"
      "0000000000000000000");
...
TEST(Arithmetic, sub_long) {
  ...
  BigInteger b("100000000000000000000000000000000000000");
  BigInteger c(
      "999999999999999999999999999999999999999999999999999990000000000000000000"
      "0000000000000000000");
...
TEST(Arithmetic, operator_mul_10_times) {
  ...
  EXPECT_EQ("375F00", to_hex(x));

  BigInteger y = 1;
  for (int i = 11; i <= 20; ++i) {
    y *= i;
  }
  EXPECT_EQ("9C197DCC00", to_hex(y));

  EXPECT_TRUE(x * y == answer);
}

TEST(Arithmetic, operator_mul_30_times) {
  BigInteger f10 = 1;
  for (int i = 2; i < 11; ++i) {
    f10 *= i;
  }
  ...
  EXPECT_EQ("D13F6370F96865DF5DD54000000", to_hex(f30));
}
